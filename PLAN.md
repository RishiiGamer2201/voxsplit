...
- [x] Web demo: `demo/app.py`, a Gradio app. Upload a recording, get for each auto-detected speaker a playable track, an after-separation spectrogram, and a transcript, plus the input spectrogram and a speaking timeline. The count is discovered (no count supplied); the CLEAN 20k OR-PIT checkpoint is the default because the count/stop classifier was trained on its residuals (the robust checkpoint separates noisy audio better but mis-counts clean input, pass it via --orpit-ckpt). The app exposes a "Split sensitivity" slider (leans the stop threshold when Auto miscounts) and a "Speaker count" dropdown to FORCE an exact count (1..8), which skips the shaky classifier and is the reliable route for the hard 4-speaker level. The blind recursion cap is decoupled from the classifier's 5-way head (`MAX_RECURSION_DEPTH = 8`), so it can be pushed past 5 for experimentation, though beyond 5 is out of the trained range (poor count and separation). `demo/pipeline.py` holds the reusable end-to-end `Pipeline` (models loaded once) shared by the app and the CLI. Verified: the server builds and serves, and the pipeline runs end to end (2-5 speakers, plus forced-count)
...
- [x] Audio-visual mode (the L2L homage): built and validated as a WORKING feature in `src/av/`. Rather than a from-scratch AV-masking model (RTFS-Net etc., which needs an external repo + video training data), VoxSplit uses the video to DRIVE the audio-only separator: on-screen faces give the speaker count (so separation is forced to that many speakers, bypassing the shaky audio-only classifier), and each separated track is matched to its on-screen speaker by correlating the track's loudness envelope with each face's mouth motion (Hungarian one-to-one). Per-face mouth motion comes from mediapipe FaceMesh when the legacy Solutions API is present, else a ROI-motion fallback that works on any video. Validated end to end on a synthetic 2-speaker talking-face clip (`make_synth_av.py`): tracks assigned to the correct faces at 19.7 / 19.6 dB SI-SDR vs ~-36 dB for the wrong face. Components each self-tested (`av_assign.py`, `lipmotion.py`, `separate_av.py`). True AV masking with a pretrained AV model remains the documented heavier alternative

## Phase 7, Evaluation, report, presentation (final 1 to 2 weeks)
- [x] Full metric sweep: SI-SDRi, PESQ, STOI per level (2-5 spk, clean/noise/reverb), baselines vs our models, consolidated by `experiments/make_report.py` into `experiments/RESULTS.md` and plots in `experiments/plots/` (quality vs speaker count, robustness, blind count accuracy, training-length ablation)
- [x] Ablations (data-driven from the committed CSVs): pretrained vs fine-tuned (out-of-domain OR-PIT gains, in-domain libri3mix does not), 6k vs 20k training length, and the count/stop classifier training domain (clean 0.25 -> single-pass residual 0.49 -> multi-level residual 0.71 blind count accuracy), plus the chunk-stitching finding (Hungarian one-to-one vs free clustering). All in RESULTS.md
- [x] Failure analysis: 4-speaker is the count weak point (0.40); >5 speakers is untrained (forced 6-speaker collapses to ~3.0 dB mean SI-SDRi); similar (same-gender) voices mis-stitch under ECAPA; reverb is the harshest degradation; the MetricGAN+ enhancement post-filter hurts. In RESULTS.md and REPORT.md
- [x] README, report, live demo: `REPORT.md` (narrative report), `experiments/RESULTS.md` (auto tables + plots), README results + demo quickstart, and the verified Gradio demo. Rehearsal: use a teammate-made surprise mixture or `src/mixing/make_mixture.py` with a fresh seed as the live-demo input
