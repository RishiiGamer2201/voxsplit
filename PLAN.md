...
- [x] Audio-visual mode (the L2L homage): scoped SCAFFOLD built in `src/av/` (README + `separate_av.py`). It reads video frames + audio (PyAV), defines the two components a full AV system needs (a `FaceCropper` and an `AVSeparator`), and falls back to the audio-only blind pipeline on the video's audio so the entry point works today. A validated end-to-end AV model is NOT built: it needs an external AV repo (RTFS-Net/IIANet/CTCNet), a face-detection/mouth-crop pipeline, and video data (AVSpeech/LRS) — a separate multi-day effort the plan gates on "after Phases 1-5" and which the audio-only evaluation does not require. The scaffold documents the exact steps to finish it. Self-tested (interfaces + fallback wiring)
